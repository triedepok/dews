 ...
 * @author trie
 */
public class ews_generate {

    private DBAdmin db;
    private Connection conn;
    private ResultSet rs;
    private String query;
    String FolderUser,_id_ws;
    File file;
    FileWriter fw;
    String network_name, ews_service_name, psi_service_name, folder_video;
    int ews_pmt_id, psi_pmt_id, network_id, psi_service_number, ews_service_number, ews_data_id,version;
    int ews_bitrate, null_bitrate, video_bitrate, psi_bitrate, psi_audio_id, psi_video_id;
    int status_player, status_muxer, card_port, stream_port, freq, bitrate_max,conten;
    String card_type, stream_ip;
    ...
    public ews_generate(String folderuser, String id_ws) {
    ...
    private void RowDataHardware(){
        ...
        try {
            ...
            query = "select * from ews_config where id_ws="+_id_ws;
            rs = db.getRS(query);
            while (rs.next()) {
                //Global
                ...
                null_bitrate = rs.getInt("null_bitrate");
                //Konten
                ...
                folder_video = rs.getString("folder_video");
    ...
    private void RowDataEws() {
        ...
        try {
            ...
            jum_section_trdw = 0;
            query = "select * from trdw where id_ws="+_id_ws;
            rs = db.getRS(query);
            while (rs.next()) {
            ...
            jum_section_tmdw = 0;
            query = "select * from tmdw where id_ws="+_id_ws;
            rs = db.getRS(query);
            while (rs.next()) {
            ...
            last_section_number_tcdw.clear();
            jum_section_tcdw = 0;
            query = "select * from tcdw where id_ws="+_id_ws;
            rs = db.getRS(query);
            while (rs.next()) {
    ...
    public void run() throws IOException, InterruptedException {
        ...
        while (true) {
                ...
                if(status_muxer == 1){
                   ...
                   psi_generate PsiStart = new psi_generate(FolderUser,network_id,psi_service_number,psi_service_name,ews_service_number,ews_service_name,psi_pmt_id,ews_pmt_id,ews_data_id,psi_video_id,psi_audio_id);
                   ...
                   fw.close();
                    ...
                    cmd.command("sh", "-c", "chmod +x " + FolderUser + "temps/ewsconfig.py; "+ FolderUser + "temps/ewsconfig.py");
                    cmd.start();
                        
                    // Jalankan muxing data ews
                    //player EwsUpdate = new player(FolderUser,ews_bitrate,null_bitrate,status_player,status_muxer,jum_section_trdw,jum_section_tcdw,jum_section_tmdw);
                    

                    //Proses player
                    if(DtaPlay == true){
                        Thread.sleep(100);
                        // Jalankan player dengan DTA Card / IP Stream
                        player PlayerStart = new player(FolderUser,conten,folder_video,psi_bitrate,ews_bitrate, null_bitrate,video_bitrate,psi_video_id,psi_audio_id,status_player,status_muxer,card_type,card_port,freq,jum_section_trdw,jum_section_tcdw,jum_section_tmdw);
                        DtaPlay = false;
                        // Update status player pada database
                        db = new DBAdmin(FolderUser);
                        try {
                            query = "UPDATE ews_config SET status_player=1 WHERE id_ws="+_id_ws;
                            db.executeRS(query);
                        } catch (SQLException ex) {
                            //Logger.getLogger(ews_generate.class.getName()).log(Level.SEVERE, null, ex);
                        }
                    ...
                    if(DtaPlay == false){
                        // Proses stop DTA Card
                        player PlayerStart = new player(FolderUser,card_type);
                        //proses shutdown PC
                        //Runtime runtime = Runtime.getRuntime();
                        //Process proc = runtime.exec("sudo sleep 20;shutdown -h now");
                        // Update status player pada database
                        db = new DBAdmin(FolderUser);
                        try {
                            query = "UPDATE ews_config SET status_player=0 WHERE id_ws="+_id_ws;
                            db.executeRS(query);
                        } catch (SQLException ex) {
                            //Logger.getLogger(ews_generate.class.getName()).log(Level.SEVERE, null, ex);
                        }
